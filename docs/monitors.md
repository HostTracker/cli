...
```
ht-cli monitors bulk-create [flags]
```
...
## `ht-cli monitors bulk-delete`

Delete every monitor a filter selects, as an asynchronous job.

```
ht-cli monitors bulk-delete [flags]
```
...
## `ht-cli monitors bulk-delete-validate`

Check which monitors a delete filter selects, without deleting anything.

```
ht-cli monitors bulk-delete-validate [flags]
```
...
## `ht-cli monitors bulk-update-validate`

Check which monitors a bulk edit would touch, without changing anything.

```
ht-cli monitors bulk-update-validate [flags]
```
...
## `ht-cli monitors get-attached`

Read the results of a monitor's attached sub-checks.

```
ht-cli monitors get-attached <monitor-id> [flags]
```
...
## `ht-cli monitors list-span`

List one monitor's up/down state spans in a window.

```
ht-cli monitors list-span <monitor-id> [flags]
```
...
## `ht-cli monitors mute-dnsbl`

Mute or unmute named blacklist zones on a monitor's blacklist check.

```
ht-cli monitors mute-dnsbl <monitor-id> [flags]
```
...
## `ht-cli monitors reset-stats`

Reset one monitor's accumulated uptime statistics.

```
ht-cli monitors reset-stats <monitor-id> [flags]
```
...
## `ht-cli monitors validate-cron`

Check a cron schedule before saving it, and see when it would run.

```
ht-cli monitors validate-cron [flags]
```
