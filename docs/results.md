...
## `ht-cli results get-monitor`

Get one check result in full detail.

```
ht-cli results get-monitor <monitor-id> <id> [flags]
```
...
## `ht-cli results get-monitor-snapshot`

Download the page snapshot captured for a check result.

```
ht-cli results get-monitor-snapshot <monitor-id> <id> [flags]
```
...
## `ht-cli results get-summary`

Get uptime, SLA and response-time figures across monitors.

```
ht-cli results get-summary [flags]
```
...
## `ht-cli results list-monitor`

List one monitor's raw check results.

```
ht-cli results list-monitor <monitor-id> [flags]
```
